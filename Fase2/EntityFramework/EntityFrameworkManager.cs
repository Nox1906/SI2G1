        ...
        public void changeCompetenciaFunc(int idFunc1, int idFunc2, int idCompFunc1, int idCompFunc2)
        {
            ...
            using (ts)
            {
                using (L51NG1Entities ctx1 = new L51NG1Entities())
                {
                    ...
                    //var funcionario1 = ctx1.
                    //.Where(f => f.id == idFunc1).FirstOrDefault();

                    using (L51NG1Entities ctx2 = new L51NG1Entities())
                    {
                        var funcionario = ctx1.Funcionarios.Where(f => f.id == idFunc1).FirstOrDefault();
                        var funcionario2 = ctx1.Funcionarios.Where(f => f.id == idFunc2).FirstOrDefault();
